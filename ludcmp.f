...
C
C DOUBLE PRECISION A(NDIM,NDIM) -> matrix of coefficients
...
C DOUBLE PRECISION SCALEROW(N) -> vector holding scaling factors applied 
C                to each row
...
C local variables
        INTEGER I,J,K
        DOUBLE PRECISION AMIN,AMAX,ATEST
...
C
        DO I=1,N
          ...
          IF(AMAX.EQ.0.D0)THEN
            ...
            IOK=I
            RETURN
          ELSEIF(AMIN.EQ.0.D0)THEN
            LSCALE=.TRUE.
          ELSE
            IF(DLOG10(AMAX)-DLOG10(AMIN).GT.1.5D0) LSCALE=.TRUE.
          END IF
          SCALEROW(I)=1.D0/AMAX
        END DO
C
        IF(LSCALE)THEN
          DO I=1,N
...
C calculamos una fila de la matriz U
          DO K=J+1,N
            DSUM=0.D0
            DO I=1,J-1
              DSUM=DSUM+A(J,I)*A(I,K)
            END DO
            A(J,K)=(A(J,K)-DSUM)/A(J,J)
          END DO
        END DO
C finalmente, calculamos el ultimo elemento de la matriz L
        DSUM=0.D0
        DO K=1,N-1
          DSUM=DSUM+A(N,K)*A(K,N)
        END DO
        A(N,N)=A(N,N)-DSUM
...
C------------------------------------------------------------------------------
        LOK=.TRUE.
C
        IF(DABS(A).EQ.0.D0)THEN
          WRITE(*,101)'ERROR: singular matrix.'
          WRITE(*,100)'Press <CR> to continue...'
          READ(*,*)
          LOK=.FALSE.
        ELSEIF(DABS(A).LT.1.D-15)THEN
