C
C******************************************************************************
C Ajusta un polinomio generico por minimos cuadrados, con errores en Y (ver,
C por ejemplo, Bevington)
	SUBROUTINE FITPOLEY(N,X,Y,EY,NDEG,IFCOEF,B,VARB,CHISQR,SR2,
     >   COVAR)
	IMPLICIT NONE
C
...
C------------------------------------------------------------------------------
        IF(N.GT.NDATAMAX)THEN
          STOP 'FATAL ERROR: N.GT.NDATAMAX in FITPOLEY'
        END IF
        IF(NDEG.GT.NDEGMAX)THEN
          STOP 'FATAL ERROR: NDEG.GT.NDEGMAX in FITPOLEY'
        END IF
C------------------------------------------------------------------------------
C pesos
	SUMERR2=0.D0
...
C
	DO K=1,N
	  WY(K)=(1.D0/(EY(K)*EY(K)))/(SUMERR2/DBLE(N))
	END DO
C------------------------------------------------------------------------------
	    ...
	    DO J=1,NDEG+1
	      IF(IFCOEF(J))THEN
	        ...
	        ELSE
	          A(JJ,II)=0.D0
	          DO K=1,N
	            A(JJ,II)=A(JJ,II)+(X(K)**(I+J-2))*WY(K)
	          END DO
	        END IF
	      END IF
	    END DO
...
C varianza residual y Chi-cuadrado
	...
	END DO
	CHISQR=SR2
	SR2=DBLE(N)*SR2/(DBLE(N-(NDEG+1))*SUMERR2)
C------------------------------------------------------------------------------
